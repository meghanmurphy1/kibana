...
export const githubOAuthProxyRoutes = (router: IRouter) => {
  // Start OAuth flow
  router.post(
    {
      path: `${INTERNAL_BASE_STACK_CONNECTORS_API_PATH}/github/oauth/start`,
      security: {
        authz: {
          enabled: false,
    ...
    async (
      ...
      req: KibanaRequest<{ scope?: string[] }, unknown, unknown>,
  ...
  router.get(
    {
      path: `${INTERNAL_BASE_STACK_CONNECTORS_API_PATH}/github/oauth/secrets`,
      security: {
        authz: {
          enabled: false,
          reason: 'This route proxies OAuth requests to an external service',
